<x-app-layout>
    <section class="bg-gray-700 py-12 mb-12">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid grid-cols-1 lg:grid-cols-2 gap-6">
            <figure>
                <img class="h-60 w-full object-cover" src="{{ $course->image_url }}" alt="">
            </figure>
            <div class="text-white">
                ...
                <p class="mb-2"> <i></i> Categoria: {{ $course->category->name }}</p>
            ...
            {{--<section class="mb-12">
                ...
                @foreach ($course->sections as $section)
                    <article class"mb-4 shadow" 
                        ...
                        @endif
                        >
                        <header class="border border-gray-200 px-4 py-2 curso-pointer bg-gray-200" x-on:click="open = !open">
                ...
                <div class="px-6 py-4">
                    ...
                    @can('enrolled', $course)
                        <a href="{{ route('courses.status', $course) }}"
                            class="font-bold py-2 px-4 rounded bg-red-500 text-white block text-center w-full mt-4">
                            Continuar con el Curso</a>
                    @else
                        <form action="{{ route('courses.enrolled', $course) }}" method="POST">
                            @csrf
                            <button type="submit"
                                class="font-bold py-2 px-4 rounded bg-red-500 text-white block text-center w-full mt-4">
                                Llevar este curso</button>
                        </form>
                    @endcan
            ...
            <aside class="hidden lg:block">
                @foreach ($similares as $similar)
                    <article class="flex mb-6">
                        <img class="h-32 w-40 object-cover" src="{{ $similar->image_url }}" alt="">
                        <div class="ml-3">
                            <h1>
